...
f_handler = handlers.TimedRotatingFileHandler(
    f"{PATHS['LOG_DIR']}/f2.log", when="midnight", backupCount=15
)
...
def ingestion():
    ...
    if last_date is None:
        ...
        logger.info(
            f"Não há dados no banco de dados, coletando a partir de {last_date}"
        )
...
def predict():
    ...
    lastDatePredicted = (
        lastDatePredicted
        if lastDatePredicted is not None
        else datetime.date(2022, 1, 1)
    )
    ...
    if (lastDatePredicted > lastDateIbov) & (
        lastDatePredicted >= datetime.datetime.now().date()
    ):
        logger.info(
            "Última data de predição: %s. Não há necessidade de novas predições",
            lastDatePredicted,
        )
        return None
    ...
    if lastDateReturn >= lastDatePredicted:
        pred = Predictor(data_ibovespa).predict(n_periods=1)
    else:
...
def consolidate():
    ...
    if end_date is None:
        ...
        logger.info(
            "Não há dados de predição. Usando Ibovespa como data final: %s", end_date
        )
    ...
    while start_date <= end_date:
        ...
        consolidate = Consolidate(date=start_date, close=ibov, predict=pred)
