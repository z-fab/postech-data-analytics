...
class Database:
    ...
    def getDataIbov(self, start_date = None, end_date = datetime.datetime.now().strftime('%Y-%m-%d')):
        ...
        with self.engine.connect() as conn:
            result = conn.execute(db.text(query), {'start_date': start_date, 'end_date': end_date})
            data = result.fetchall()
            if data:
                logger.info('Retornando %s registros do banco de dados [ Start %s | End %s ]', len(data), data[0][0], data[-1][0])
                return data
            else:
                logger.info('Nenhum dado retornado do banco de dados [ Start %s | End %s ]', start_date, end_date)
                return None

            
    def getDataPred(self, start_date = None, end_date = datetime.datetime.now().strftime('%Y-%m-%d')):
        ...
        with self.engine.connect() as conn:
            ...
            data = result.fetchall()
            if data:
                logger.info('Retornando %s registros de predict [ Start %s | End %s ]', len(data), start_date, end_date)
                return data
            else:
                logger.info('Nenhum dado retornado de predict [ Start %s | End %s ]', start_date, end_date)
                return None
            ...
            with self.engine.connect() as conn:
                ...
                if isinstance(data, list):
                    ...
                    for consolidate in data:
                        logger.info('Inserindo registro de %s em Consolidate', consolidate.date)
                        conn.execute(db.text(query), {
