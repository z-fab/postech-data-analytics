...
def getDataIbov(delta=365) -> pd.DataFrame:
    ...
    df = db.getDataIbov(
        start_date=datetime.datetime.now().date() - datetime.timedelta(days=delta)
    )
...
def graph_pred(delta=14, title="") -> None:
    ...
    fig = px.area(
        ...
        color="variable",
        title=title,
        color_discrete_map={"IBov": "#457b9d", "Predição": "#e63946"},
    )
...
def table_ibovespa() -> None:
    ...
    df_final[["close", "predict", "diff_percent", "diff"]] = df_final[
        ["close", "predict", "diff_percent", "diff"]
    ].astype(str)

    df_final["predict"] = df_final["predict"].apply(
        lambda x: "-" if x == "0" else f"{x} pts."
    )
    df_final["close"] = df_final["close"].apply(
        lambda x: "-" if x == "0" else f"{x} pts."
    )
    df_final["diff_percent"] = df_final[["close", "diff_percent"]].apply(
        lambda x: "-" if x["close"] == "-" else f"{x['diff_percent']}%", axis=1
    )
    df_final["diff"] = df_final[["close", "diff"]].apply(
        lambda x: "-" if x["close"] == "-" else f"{x['diff']} pts.", axis=1
    )
    ...
    df_final = df_final.rename(
        columns={
            ...
            "predict": "Previsão",
            "date": "Data",
            "diff": "Diferença (pontos)",
            "diff_percent": "Diferença (%)",
        }
    )

    st.dataframe(
        df_final[["Pontuação", "Previsão", "Diferença (pontos)", "Diferença (%)"]],
...
def indicators_ibovespa():
    ...
    cols[0].metric(
        label=":gray[Última atualização IBovespa]", value=last_date.strftime("%d/%m/%Y")
    )
    cols[1].metric(
...
def desc_project():
    st.info(
        "Para ver o conteúdo completo e o código do Dashboard acesse o repositório do projeto em https://github.com/z-fab/postech-data-analytics/tree/master/tech-challenge/f2-previsao-ibovespa",
        icon="ℹ️",
    )
    ...
    content = f"""
        ...
        O Dashboard foi criado utilizando o Streamlit. Existe uma rotina que atualiza diariamente os dados e, quando necessário, realiza a previsão para odia seguinte.
